...
class Graph:
    ...
    def splashTime(self):
        return debugger.splash_time(self.handler,self.height,self.width)
...
class Vertex:
    ...
    def calculateAbsoluteSize(self,text):
        theight=0
        for line in text:
            (twidth,theight)=debugger.get_text_size(self.handler,line)
            if twidth > self.absx:
                self.absx=twidth
...
class Draw:
    ...
    def createGraphWindow(self,title,start_address):
        ...
        self.handler=debugger.create_graph_window(title,self.start_address)
        return self.handler
...
class vcgNode:
    ...
    def drawRect(self,x1,y1,x2,y2,color):
        ...
        self.y2=self.y2
        self.color=color
        return debugger.draw_rectangle(self.handler,x1,y1,x2,y2,ImmDrawColors[self.color])
