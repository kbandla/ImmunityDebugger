...
class Directory:
    ...
    def get(self, data):
        (self.VirtualAddress, self.Size) = struct.unpack("2L", data[:8])
...
class ImageExportDirectory:
        ...
        def get(self, data):
                (self.Characteristics, self.TimeDateStamp, self.MajorVersion, self.MinorVersion, self.Name, self.Base,\
                    self.NumberOfFunctions, self.NumberOfNames, self.AddressOfFunctions, self.AddressOfNames, \
                    self.AddressOfNameOrdinals) = struct.unpack(self.fmt, data[:struct.calcsize(self.fmt)])
...
class Section:
        ...
        def get(self, data):
                ...
                (self.VirtualSize, self.VirtualAddress, self.SizeOfRawData, self.PointerToRawData ,\
                                ...
                                struct.unpack(self.fmt, data[idx:idx+struct.caclsize(self.fmt)])
...
class IMGhdr:
        ...
        def get(self, data):
             try:
                ...
                self.NumberOfSymbols,\
                self.SizeOfOptionalHeader,\
                self.Characteristics)=struct.unpack(self.imagefmt, data[:struct.calcsize(self.imagefmt)])
             except struct.error:
                raise PEError, "Invalid IMAGE header" 
        ...
        def get(self, data):    
            try:
                ...
                self.MajorLinkerVersion,\
                    ...
                    self.SizeOfHeapCommit,\
                    self.LoaderFlags,\
                    self.NumberOfRvaAndSizes )= struct.unpack(self.optionalfmt, data[:struct.calcsize(self.optionalfmt)])
            except struct.error:
