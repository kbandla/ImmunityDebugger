...
class StateMachine:
    ...
    def __getstate__(self):
        ...
        RETNoffset=self.solver.dumpExpr(self.RETNoffset)
        
        return cPickle.dumps((tmp1,tmp2a,tmp2b,tmp2c,tmp3,tmp4,EIP,RETNoffset), cPickle.HIGHEST_PROTOCOL)
    
    def __setstate__(self, state):
        (tmp1,tmp2a,tmp2b,tmp2c,tmp3,tmp4,EIP,RETNoffset) = cPickle.loads(state)
        ...
        self.RETNoffset=self.solver.loadExpr(RETNoffset)
    ...
    def initState(self, r_model=None, f_model=None):
        ...
        self.RETNoffset = self.solver.varExpr("RETNOffset")
    ...
    def calcComplexity(self, regProps, flagProps):
        ...
        if tmp == None:
            tmp = self.solver.UConstFromExpr(self.RETNoffset)
            if tmp != None:
                tmp+=4 #for the RETN itself
        if tmp != None:
    ...
    def push(self):
        ...
        tosave = ( deepcopy(self.regs), deepcopy(self.flags), deepcopy(self.exceptions), deepcopy(self.memory.sources),\
                   deepcopy(self.memory.undefvalues), self.memory.copy(), self.EIP, self.RETNoffset, deepcopy(self.callstack) )
    ...
    def pop(self):
        ...
        #now the VM
        ret = self.stack.pop()
        ( self.regs, self.flags, self.exceptions, self.memory.sources, self.memory.undefvalues, mem, self.EIP, self.RETNoffset, self.callstack ) = ret
    ...
    def mergeState(self, statemachine):
        ...
        #merge RETNoffset
        dump = statemachine.solver.dumpExpr(statemachine.RETNoffset)
        self.RETNoffset = self.solver.simplify(self.solver.loadExpr(dump, varsdict=sub))
    ...
    def simplify(self):
        ...
        for key in self.flags.keys():
            self.flags[key] = self.solver.simplify(self.flags[key])
        ...
        self.RETNoffset = self.solver.simplify(self.RETNoffset)
    ...
    def prettify(self):
        ...
        self.RETNoffset = Expression(self.solver, self.RETNoffset)
    ...
    def printRegisters(self, imm):
        ...
        for k,v in self.exceptions.iteritems():
            imm.log("%s:%s"%(k,self.solver.exprString(v)))
        imm.log("RETN offset:%s"%self.solver.exprString(self.RETNoffset))
...
class SequenceAnalyzer:
    ...
    def analyzeRETN(self, op):
        ...
        self.state.RETNoffset = self.state.solver.constExpr(op.op1Constant())
        self.state.regs['ESP'] = self.state.solver.addExpr(self.state.regs['ESP'], self.state.solver.addExpr(self.state.solver.constExpr(4), self.state.RETNoffset))
    ...
    def analyzeJcc(self, condition, finaladdress):
        ...
        #(regs, flags, exceptions, memsources, undefvalues, mem, eip, retnoffset)
        ...
        if self.compareWithCondition(condition, trueState[6], falseState[6]):
            self.state.RETNoffset = falseState[7]
        else:
            self.state.RETNoffset = self.state.solver.iteExpr(condition, trueState[7], falseState[7])
    ...
    def setMemoryStateFromSolverState(self, state, value, size):
        ...
        if self.useCurrentMemory:
            tmp1 = self.getAddressFromState(state) #just for raising an exception if the address is not constant
            
        if self._debug:
            tmp2 = self.state.solver.UConstFromExpr(self.state.solver.simplify(value))
            if tmp2 == None: tmp2=self.state.exprString(self.state.solver.simplify(value))
            else: tmp2 = "0x%x"%tmp2
            
            self.imm.log("Writing to 0x%08x, size: %d bytes, value=%s"%(tmp1, size/8, tmp2))
