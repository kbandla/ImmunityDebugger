...
class GFlags:
    ...
    def Set(self, val):
        ...
        for a in NeedPageHeap:
            if val == a:
                self._set( NeedPageHeap[a], heapf = True)
                break
    ...
    def UnSet(self, val):
        ...
        for a in NeedPageHeap:
            if val == a:
                hkey = _winreg.OpenKey(_winreg.HKEY_LOCAL_MACHINE, self.subkey, 0, _winreg.KEY_ALL_ACCESS)
                _winreg.DeleteValue(hkey, "PageHeapFlags")
    ...
    def _set(self, flag, heapf = False):
        try:
        ...
        try:
            _winreg.SetValueEx(hkey, ("GlobalFlag", "PageHeapFlags")[heapf], 0, _winreg.REG_DWORD, int(flag) )                
        except WindowsError:
